...
export function SearchDropdown({
  ...
  const resolveSubmittedId = () => {
    ...
    const exactMatch = safeOptions.find(
      (opt) => normalizeId(opt.id) === normalizedSearch,
    );
  ...
  return (
    <div className="relative w-full max-w-md">
      <div className="flex items-center gap-2">
        <input
          ...
          onChange={(e) => {
            ...
            if (
              nextSearch === "" ||
              normalizeId(nextSearch) !== normalizeId(value || "")
            ) {
              setValue("");
            }
          }}
      ...
      {open && filteredOptions.length > 0 && (
        <div className="absolute left-0 w-full mt-1 bg-slate-800 border border-slate-600 rounded-md shadow-lg z-50 max-h-60 overflow-y-auto">
          {filteredOptions.map((opt) => (
            <div
              ...
              className={cn(
                "px-3 py-2 text-sm cursor-grab text-slate-100 hover:bg-slate-700 hover:text-white",
      ...
      {open && search.length > 0 && filteredOptions.length === 0 && (
        <div className="absolute left-0 w-full mt-1 bg-slate-800 border border-slate-600 rounded-md shadow-lg z-50 p-3 text-sm text-slate-400">
          No modules found
        </div>
      )}
    </div>
